...
class PipeSettingsMdl(bpy.types.PropertyGroup):
    ...
    spacing: FloatProperty(
        name='Spacing',
        description='Space between variations in the exploded alembic',
        default=2.0)
...
class MdlUiExport(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row1.prop(pipe_tool, 'include_hidden')
        ...
        if asset not in bpy.data.collections:
            ...
            # ACTUALLY EXPORT
                ...
                bpy.ops.wm.alembic_export(filepath=full_path_abc_txt_additions_bake, selected=True, uvs=True,
                                          packuv=False, face_sets=True, use_instancing=False)
                ...
                bpy.ops.wm.alembic_export(filepath=full_path_abc_txt_exp, selected=True, uvs=True, packuv=False,
                                          face_sets=False, use_instancing=False)
...
class AllUiTurntable(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if scene_has_been_loaded is False or (scene_has_been_loaded is True and scene_is_turntable is True):
            ...
            row2 = layout.row()
            if scene_has_been_loaded:
                # button to update asset, (needs to update shading file, modelling file and texturing)
                # button to update turntable settings
                #row2.label(text='UPDATE TODO')
                row2.operator('pipe.turntable_update')
                #row2.operator('pipe.turntable_update_settings')
            else:
            ...
            row1.label(text='Tool deactivated')
            row1.enabled = False
...
class AllTurntableUpdate(bpy.types.Operator):
    ...
    def __init__(self):
        self.hdri_1_bool = self.hdri_2_bool = self.hdri_3_bool = self.hdri_4_bool = self.hdri_5_bool = \
            ...
            self.frames = self.asset_yaw = self.asset_pitch = self.render_flat_textures = self.render_shading = None
    ...
    def execute(self, context):
        ...
        json_file_version = os.path.join(dir_pipe_shd_variation, 'versions', f'{asset}.{new_shd_version}.json')

        '''with open(json_file_version, 'r') as json_content:
            new_shd_version_blend = json.load(json_content)['version']'''

        # Get SHD version
        libraries = [x for x in bpy.data.libraries]
        current_shd_version = context.window.scene.name.split('.')[-1]

        '''for library in libraries:
            dir_parts = library.filepath.split(os.sep)
            for i, x in enumerate(dir_parts):
                if x == asset:
                    if dir_parts[i + 1] == 'shd':
                        current_shd_version = dir_parts[i + 4]
                        break
        ...
        print(new_shd_version, current_shd_version)
        if new_shd_version != current_shd_version:
            '''if current_shd_version is not None:
                libraries = [x for x in bpy.data.libraries]
                for library in libraries:
                    dir_parts = library.filepath.split(os.sep)
                    for i, x in enumerate(dir_parts):
                        if x == asset:
                            if dir_parts[i + 1] != 'shd' or new_shd_version_blend not in library.name:
            ...
            if self.hdri_1_bool or self.hdri_2_bool or self.hdri_3_bool or self.hdri_4_bool or self.hdri_5_bool or \
                ...
                # -- UPDATE MODEL --
                # UPDATE ASSET
                AllTurntableFunctions().update_asset()
            ...
            # copy ui settings (including render settings? and other windows?)
            # rename old scene
            # call operator of scene setup
            # delete old scene and old shd library
        ...
        for obj in objects_of_asset:
            ...
            for i, material_slot in enumerate(obj.data.materials):
                ...
                material_slot.use_fake_user = False

                if material_slot not in materials_of_asset:
                    materials_of_asset.append(material_slot)

        print("materials_of_asset", materials_of_asset)

        old_mats_new_names = []
        for mat in materials_of_asset:
            prefix = mat.name
            if '.' in mat.name:
                maybe_version = mat.name.split('.')[-1]
                print('AAA', mat.name)
                print("maybe_version", maybe_version[1:])
                if maybe_version[0] == 'v' and maybe_version[1:].isdigit():
                    print('xxx', '.'.join(mat.name.split('.')[:-1]))
                    prefix = '.'.join(mat.name.split('.')[:-1])
            new_name = f'{prefix}.{current_shd_version}'
            mat.name = new_name
            old_mats_new_names.append(new_name)

        dir_shd_version = os.path.join(dir_asset, 'shd', '.pantry', new_shd_version_blend)
        blend_shd_version = os.path.join(dir_shd_version, f'{asset}.blend')

        for mat in materials_of_asset:
            try:
                #blend_shd_mat = os.path.join(blend_shd_version, 'Materials', mat)
                bpy.ops.wm.append(directory=f'{blend_shd_version}\\Materials\\', filepath=f'{asset}.blend',
                                  filename='.'.join(mat.name.split('.')[:-1]), autoselect=True)
            except RuntimeError:
                pass'''


        # import materials from latest shading file
        # assign new materials to objects
        # delete old materials
        # default model and texture update
...
# ONLY UPDATES MODEL AND TEXTURES NOT SHADING FILE
class AllTurntableUpdateAssetOld(bpy.types.Operator):
    bl_idname = 'pipe.turntable_update_asset_old'
    ...
    def execute(self, context):
        ...
        if self.hdri_1_bool or self.hdri_2_bool or self.hdri_3_bool or self.hdri_4_bool or self.hdri_5_bool or \
            ...
            self.report({'ERROR'}, 'Scene has not been updated as no HDRIs have been selected and flat rendering is '
                                   'disabled.')
            return {'CANCELLED'}


class AllTurntableUpdateSettings(bpy.types.Operator):
    bl_idname = 'pipe.turntable_update_settings'
    bl_label = 'Update Settings'
    bl_options = {'REGISTER', 'UNDO'}
    bl_description = 'Updates the turntable according to the settings below'
...
class AllSetupTurntable(bpy.types.Operator):
    ...
    def __init__(self):
        self.hdri_1_bool = self.hdri_2_bool = self.hdri_3_bool = self.hdri_4_bool = self.hdri_5_bool = \
            self.hdri_1 = self.hdri_2 = self.hdri_3 = self.hdri_4 = self.hdri_5 = self.hdri_1_rotation = \
            self.hdri_2_rotation = self.hdri_3_rotation = self.hdri_4_rotation = self.hdri_5_rotation = \
            self.frames = self.asset_pitch = self.asset_yaw = self.render_shading = self.render_flat_textures = None
...
# TODO creating a project needs to create default configs (subdivision)
...
def convert_alembic_input():
    objects = [[x, x.name] for x in bpy.data.objects]
    print()
    ...
    for obj, name in objects:
        name_mod_1 = name.split(':')[1].replace('___', '_*_')
        # print(name_mod_1)
...
# TODO is model publish alembic smoothed or not?
# TODO set up collections for variations and for model version with all items from it

#convert_alembic_input()
...
class ShdUiPackageManagement(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        if scene_has_been_loaded is False or scene_is_turntable is False:
            ...
            row1.label(text='Tool disabled - turntable scenes detected')
...
class ShdSettingsVariationGet(bpy.types.Operator):
...
class ShdSettingsVariationSet(bpy.types.Operator):
...
#UNECESSARY
class AllSettingsCyclesTurntable(bpy.types.Operator):
    bl_idname = 'pipe.all_settings_cycles_turntable'
    bl_label = 'Cycles Turntable'
    bl_options = {'REGISTER', 'UNDO'}  # Enable undo for the operator.
    bl_description = 'Updates the render settings according to the default settings for this purpose'

    def execute(self, context):
        RenderSettings().cycles_turntable()
        return {'FINISHED'}


#UNECESSARY
class AllSettingsCyclesShading(bpy.types.Operator):
    bl_idname = 'pipe.all_settings_cycles_shading'
    bl_label = 'Cycles Shading'
    bl_options = {'REGISTER', 'UNDO'}  # Enable undo for the operator.
    bl_description = 'Updates the render settings according to the default settings for this purpose'
...
def create_camera():
    ...
    cam_obj.location[1] = -distance * 1.1
            ...
            if os.path.isfile(file_mdl_package) is False:
                ...
                if len(txt_var) == 1 and (txt_var.isupper() or txt_var.islower()):
                    if txt_var in txt_variations:
                        ...
                        initialise_texture_variation(dict_mdl_package, dict_mdl_variations)
...
def archive_for_update(mdl_version, current_mdl_collection):
    ...
    for obj in current_mdl_collection.objects:
        material_information = []
        for i, material_slot in enumerate(obj.data.materials):
            #if material_slot != None: # used to protect from error on updating turntable
            #try:
            material_information.append([i, material_slot])
            #if material_slot != None:  # used to protect from error on updating turntable
            ...
            #    pass
        dict_previous_objects[obj.name] = {"mat": material_information}
...
def update_model(new_mdl_package_type, dict_mdl_package, dict_mdl_variations):
    ...
    if new_mdl_package_type == 'mdl':
        ...
        if new_mdl_package_type != current_mdl_package_type \
            ...
            objects, _ = import_model(dict_mdl_package, dir_asset, dict_mdl_variations, txt_var)

            for obj in objects:
                if obj.name in dict_previous_objects and obj.data.materials:
                    materials_new = [[i, material_slot] for i, material_slot in enumerate(obj.data.materials)]
                    materials_archive = dict_previous_objects[obj.name]["mat"]
...
# TODO, subdivision should be ignored during alembic export
...
def import_model(dict_mdl_package, dir_asset, dict_mdl_variations, txt_variation):
    ...
    for col in bpy.data.collections:
        if asset in col.children:
            col.keys.unlink(bpy.data.collections[asset])
    ...
    # TODO adding ACP instead of MCP
...
# TODO allow linking of materials from other shading variations


def initialise_texture_variation(dict_mdl_package, dict_mdl_variations):
    global root, project, asset, discipline, dir_asset, dir_pipe_txt, txt_var
...
all_classes = (PipeSettingsAll, AllOpenOutput, AllUiRenderSettings, AllSettingsCyclesTurntable, AllTurntableRender,
               AllSettingsCyclesShading, AllSettingsApply, AllSetupTurntable, AllFiletypeApply,
               AllTurntableUpdateSettings,
               AllTurntableUpdate, AllUiTurntable)
mdl_classes = (MdlRemoveFrom, MdlSelectExclusive, MdlVariationDelete, MdlVariationAdd, MdlVariationApply,
               MdlExport, MdlUiVariation, MdlUiExport)
txt_classes = (ShdSettingsVariationGet, ShdSettingsVariationSet)
shd_classes = (ShdExport, ShdUpdateAsset, ShdSetup,
               ShdUiPackageManagement, ShdSettingsVariationGet, ShdSettingsVariationSet, ShdUiExport)
