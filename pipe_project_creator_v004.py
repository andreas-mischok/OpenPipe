...
class ProjectCreator:
    ...
    def hdri_add(self):
        ...
        self.hdri_list[str(index)] = {
            "ui": [i_label, i_path, bt_path_pick, cb_default, i_offset, dd_hdri_contrast, bt_delete],
        ...
        with open(config, 'r') as config_file:
            ...
            for i, hdri in enumerate(hdris):
                ...
                if hdri == hdri_default:
                    ...
                    self.hdri_list[str(i)]["default_var"].set(False)
                self.switch_hdri_type(self.hdri_list[str(i)]["ui"][-2], hdris[hdri]['contrast'])
                    ...
                    if len(dict_software) > 0:
                        ...
                        if len(hdri_default) != 0:
                            if len(hdri_labels) == len(set(hdri_labels)) and len(hdri_paths) == len(set(hdri_paths)):
                                dict_hdris = {}
                                for label, path, rotation, contrast in zip(hdri_labels, hdri_paths, hdri_offsets,
                                                                           hdri_contrasts):
                                    if label != '' and label != ' ':
                                ...
                                dir_json_output = f'.\\projects\\{name_abbr}\\'
                                file_json_output = f'.\\projects\\{name_abbr}\\{name_abbr}.json'
                                ...
                                if len(existing_projects) > 0:
                                    ...
                                    if name in existing_names or abbr in existing_abbreviations:
                                        if name in existing_names and abbr in existing_abbreviations:
                                            ...
                                            if msg == 'yes':
                                                ...
                                                self.write_file(dir_json_output, file_json_output, dict_project)
                                        else:
                                            message = "Project with same name OR abbreviation exists. " \
                                    ...
                                    # If a new name is used
                                        ...
                                        create_folder_structure(name, root)
                                        self.write_file(dir_json_output, file_json_output, dict_project)
                                        self.copy_configs(dir_json_output)
