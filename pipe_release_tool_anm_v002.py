...
class ReleaseAnim:
    ...
    def release_animations(self):
        ...
        if len(animations_for_export) > 0:
            for animation in animations_for_export:
                ...
                file_animation_src = os.path.join(self.parent.dir_export_anm, f'{animation}.fbx')
                file_animation_trgt = os.path.join(dir_pantry_version_current, f'{animation}.fbx')
                ...
                os.putenv('PIPE_FBX', file_animation_src)
                os.putenv('PIPE_OUTPUT', dir_pantry_version_current)
                os.putenv('PIPE_ANIMATION', animation)
                os.putenv('PIPE_SCALE', self.tkvar_scale.get())

                exe = self.parent.dict_software[blender]["exe"]
                #cmd = f'{exe} --background --factory-startup --python ./pipe_blender_convert_fbx_v001.py'
                cmd = f'{exe} --factory-startup --python ./pipe_blender_convert_fbx_v001.py'
    ...
    def create_ui_release_animation(self, parent):
        ...
        # --- Variations ------------------------------------------------------------------------------
        variations = [x for x in os.listdir(self.parent.dir_export_anm)
                      if os.path.isfile(os.path.join(self.parent.dir_export_anm, x))
                      and '.fbx' in x]
        ...
        # ---------------------------------------------------------------------------------------------
        lbl_source_directory = Label(self.frame_main, bd=1, text='Scale', anchor=W,
                                     bg=self.col_wdw_default, fg=self.col_bt_fg_default)
        lbl_source_directory.grid(row=2, column=2, sticky=E, padx=self.default_padding, pady=self.default_padding)


        self.tkvar_scale = StringVar()
        self.tkvar_scale.set('100')
        i_scale = Entry(self.frame_main, bd=1, textvariable=self.tkvar_scale, width=4,
                        bg=self.col_i_bg_default, fg=self.col_bt_fg_default, relief=self.def_bt_relief,
                        insertbackground=self.col_bt_fg_default)
        i_scale.grid(row=2, column=3, sticky=E, padx=self.default_padding, pady=self.default_padding)
        CreateToolTip(i_scale, "Scale compensation factor. Applied after import. Default for FBX is 100.")
        ...
        self.bt_release.grid(row=2, column=4, sticky=N, padx=self.default_padding, pady=self.default_padding)
        # ---------------------------------------------------------------------------------------------
