...
class ReleaseTextures:
    ...
    def __init__(self):
        ...
        self.var_publish = self.var_overwrite = self.path = self.path_root = self.filetypes = self.artist = \
            self.time = self.overwrite = \
            self.publish = self.connections_default = self.def_bt_relief = self.default_bt_bd = \
            ...
            self.col_wdw_title = self.parent = self.subchannels_default =None
    ...
    def json_load_variables(self, config):
        ...
        try:  # variables in config_release_tool_txt.json
            ...
            self.connections_default = config_file_content["connections"]["channels"]
            self.subchannels_default = config_file_content["connections"]["subchannels"]
        except KeyError:
            pass
    ...
    def json_create_txt_connections(self, dir_pipe_variation, variation):
        ...
        if os.path.isfile(full_path) is False:
            ...
            dict_out = {
                ...
                "subchannels": self.subchannels_default
            }
            with open(full_path, 'w') as json_output_channel:
    ...
    def release_textures(self):
        ...
        if len(variations_for_export) > 0:
            for variation in variations_for_export:
                dir_variation = os.path.join(self.parent.dir_export_txt, variation)
                dir_variation_pantry = os.path.join(self.parent.dir_pantry_txt, variation)
                if os.path.isdir(dir_variation_pantry) is False:
                    os.makedirs(dir_variation_pantry)

                dir_pipe_variation = os.path.join(self.parent.dir_pipeline, 'txt', variation)
                ...
                if len(files) > 0:
                    ...
                    for channel in channels:
                        dir_channel = os.path.join(self.parent.dir_pantry_txt, variation, channel)
    ...
    def create_ui_texture_release(self, parent):
        self.parent = parent

        #self.user = self.parent.user
        #self.current_root = self.parent.current_root
        #self.current_project_name = self.parent.current_project_name
        #self.current_department = self.parent.current_department
        #self.current_discipline = self.parent.current_discipline
        #self.current_asset = self.parent.current_asset

        generate_paths(self.parent)
        ...
        CreateToolTip(lbl_source_directory,
                      ...
                      f'{self.parent.current_asset}  /  txt  /  _export')
        # ---------------------------------------------------------------------------------------------

        # --- Variations ------------------------------------------------------------------------------
        variations = [x for x in os.listdir(self.parent.dir_export_txt) if os.path.isdir(os.path.join(self.parent.dir_export_txt, x))]
